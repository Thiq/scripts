...
export class NPCFactory {
    ...
    constructor(factory) {
        ...
        eventHandler('player', 'interactEntity', (e) => {
            ...
            if (!questNPC) return;
            if (this._factory.isQuesting(player)) {
                player.sendMessage("\xA7cYou're already engaged in a quest!");
                return;
            }
            var questNPC = this.getNPCByUUID(npc.getUniqueId());
            let nextQuest = null;
    ...
    getNPCByUUID(uuid: string): QuestNPC {
        return _.find(this._registeredNpcs, function(npc) {
            return npc.uniqueID == uuid.toString();
        });
    }

    getNPCByName(name: string): QuestNPC {
        return _.where(this._registeredNpcs, { name: name })[0];
    }
