...
var isWin32 = java.lang.System.getProperty('os.name').startsWith('Windows');
var sep = isWin32 ? '\\' : '/';
var wrapper = [
...
Module.prototype.compile = function() {
	if (this.config && this.config._jvmversion) {
		if (version.gte(this.config._jvmversion, version.getJvmVersion())) {
			throw new Error('Module ' + this.id + ' is incompitable with Java version ' + version.getJvmVersion() + ' (current version ' + version.getJvmVersion() + ')');
		}
	}
	if (this.config && this.config._bukkitversion) {
		if (version.gte(this.config._bukkitversion, version.getBukkitVersion())) {
			throw new Error('Module ' + this.id + ' is incompatible with Bukkit version ' + version.getBukkitVersion() + ' (current version ' + version.getBukkitVersion() + ')');
		}
	}
	...
	try {
		...
		ex.message = 'An error occured when reading ' + self.filename + ': ' + ex.message;
