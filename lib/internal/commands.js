...
function RuntimeScript(script) {
    this.script = '(function(require, __filename) {' + script + '\n})';
    var m = this.module = { id: 'repl', filename: process.cwd() + process.config.main, exports: {} };
    this.args = [
        function(request) {
            return _require(request, m);
        },
        m.filename
    ]
}
...
RuntimeScript.prototype.eval = function() {
    var compiled = eval(this.script);
    return compiled.apply(null, this.args);
}
...
function cmdEval(message, sender, type) {
    ...
    try {
        ...
        with(event.ext) {
            result = evalScript(runtimeScript.eval());
        }
